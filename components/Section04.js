...
export default class SectionPage extends React.Component {
  ...
  updateContract(raw, path, kind){
    console.log(raw, path, kind)
  }
  ...
  getPage(){
    let formOptions = [
      ...
      },
      {
        type: t.struct({
          vacation_days: t.Num,
          personal_days: t.Num,
          parental_leave: t.struct({
            parental_leave_notice_length: t.subtype(t.Num, n => n > 2),
            parental_leave_paid: t.Num,
            parental_leave_paid_note: t.maybe(t.Str)
          }),
          reduced_hours_reg_wage: t.Num
        }),
        options: {
          config: {
            // for each of lg md sm xs you can specify the columns width
                    ...
                    return <div className={cs}>
                      ...
                      <div className='col-sm-6 col-md-9 col-lg-9'>
                        <ul className='list-inline'>
                          <li><a className='btn btn-primary'>Yes</a></li>
                          <li><a className='btn btn-link'>No</a></li>
                        </ul>
                        <input type='hidden' name={locals.attrs.name} />
                      </div>
                    </div>
                  }
                },
                parental_leave_paid_note: {
                  label: ' ',
                  attrs: {
    ...
    let page = formOptions[pageNum];
    return <Form
      ref="form"
      type={page.type}
      options={page.options}
      onChange={this.updateContract.bind(this)}
    />
  }
